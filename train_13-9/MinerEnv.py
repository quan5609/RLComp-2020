...
class MinerEnv:
    ...
    def findBestCluster(self, agentCluster, mapId=None):
        self.new_decision = True
        if agentCluster is not None and agentCluster < self.clusterNum:
            # print("BUG", agentCluster, self.clusterNum)
            desx, desy, resultCluster = self.getClusterByIndex(
                self.state.mapInfo.clusterList, agentCluster)
            if mapId != 3:
                return desx, desy, resultCluster
            if self.mahattan(desx, desy, self.state.x, self.state.y) < 16:
                return desx, desy, resultCluster

        bestCluster = None
        globalClusterScore = -10000
        ...
        for cluster in self.state.mapInfo.clusterList:
            if cluster.total_gold > 0:
                ...
                if self.state.stepCount > 80:
                    remained_step = 100 - self.state.stepCount
                    maxgold_mining = (remained_step - mahattanDistance) * 50
                    estimateGoldInCluster = min(
                        estimateGoldInCluster, maxgold_mining)

                clusterScore = estimateGoldInCluster - 30 * mahattanDistance
    ...
    def get_cluster_score(self, cluster):
        if cluster.total_gold > 0:
            ...
            estimateGoldInCluster = cluster.total_gold - 50 * \
                cluster.checkEnermyInCluster(
                    self.state.players)*(mahattanDistance)
            clusterScore = estimateGoldInCluster - 25 * estimatePathToCluster
            clusterScore = max(0, clusterScore)
            return clusterScore
        return -100
    ...
    def get_reward(self):
        score = 0
        # # print(self.targetCluster, self.currentCluster, self.agentState)
    ...
    def get_agent_state(self,  agentCluster=None, mapId=None):
        ...
        if self.agentState == AgentState.GOCLUSTER:
            ...
            elif self.targetDesx == self.state.x and self.targetDesy == self.state.y:
                newDesx, newDesy, newCluster = self.findBestCluster(agentCluster, mapId)
                if self.targetCluster.total_gold > 100 or newCluster._id == self.targetCluster._id:
            ...
            if self.currentCluster is None or self.currentCluster.total_gold <= 100:
                newDesx, newDesy, newCluster = self.findBestCluster(
                    agentCluster, mapId)
            ...
            if self.targetDesx == self.state.x and self.targetDesy == self.state.y:
                ...
                # print("XY:", self.state.x, self.state.x)
                    ...
                    self.currentCluster = None
                    self.targetDesx, self.targetDesy = newDesx, newDesy
            elif self.currentCluster is not None:
                newDesx, newDesy, newCluster = self.findBestCluster(
                        agentCluster, mapId)
                if self.currentCluster.total_gold > 100 or newCluster._id == self.currentCluster._id:
                    self.agentState = AgentState.INCLUSTER
                else:
                    self.agentState = AgentState.GOCLUSTER
                    self.currentCluster = None
    ...
    def get_action(self):
        ...
        if self.isSleeping:
            limit = 36
            map_total_gold = sum(
                [cluster.total_gold for cluster in self.state.mapInfo.clusterList])
            if map_total_gold < 400:
                limit = 27
            if self.agentState == AgentState.MINING and self.state.mapInfo.gold_amount(self.state.x, self.state.y) < 550 and len(self.estimateBotAtCurr(self.state.x, self.state.y)) > 0:
                limit = 27
            if self.state.stepCount >= 93:
                limit = 27
            if self.state.stepCount >= 97:
                limit = 13
            if self.state.energy >= limit and energyOfBest > 0:
                self.isSleeping = False
    ...
    def estimateBotAtCurr(self, goldX, goldY):
        countBot = []
        for player in self.state.players:
            if player["playerId"] != self.state.id:
                distanceToGold = self.mahattan(
