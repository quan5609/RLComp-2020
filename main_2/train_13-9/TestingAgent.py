...
json_file = open('TrainedModels/DQNmodel_latest.json', 'r')
...
try:
    ...
    while not minerEnv.check_terminate():
        try:
            ...
            # current_state = s
            # print("State:", s)
            predicted = DQNAgent.DQN_predict(s)
            # predicted = DQNAgent.act(s)
            # clusterId = np.argmax(DQNAgent.predict(s.reshape(1, len(s))))
            clusterId = np.argmax(predicted)
            # if count_step == 0:
    ...
    print("Final Gold:", minerEnv.state.score)
    # elif agentId == 1:
    #     print("BEST 2 4")
